...
describe("Pruebas de Integración: PayasosService + Database", () => {
  ...
  test("Debe registrar un payaso nuevo y guardarlo en la base de datos", async () => {
    ...
    const result = await registerPayaso("Pedro Sánchez", "perrito@gmail.com");

    // 2. Comprobamos la respuesta del servicio
    expect(result).toHaveProperty("id");
    expect(result.name).toBe("Pedro Sánchez");
    expect(result.email).toBe("perrito@gmail.com");
    ...
    const payasoInDb = await db.findPayasoByEmail("perrito@gmail.com");
    expect(payasoInDb).not.toBeNull();
    expect(payasoInDb.name).toBe("Pedro Sánchez");
  });
